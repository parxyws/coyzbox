...
## 1. Summary / Purpose
**CozyBox** is a **multi-tenant, desktop-optimized document engine** that eliminates "folder chaos" and "blank page syndrome" for SMEs, freelancers, and micro-landlords. It provides a unified service to generate compliant business documents and manage them in a centralized, searchable repository.

In v1.0, CozyBox focuses on six standard financial document types. Each tenant operates in isolation. Documents are generated directly in the user's browser for instant feedback and saved immutably to the cloud.
...
* **Hybrid Data Sourcing:** Fetches data from the Organization Profile and Contact records, combined with manual entry fields.
* **Client-Side PDF Export:** High-quality PDF generation is executed entirely in the browser using a dedicated layout engine (`@react-pdf/renderer`).
* **Immutable Storage Upload:** Upon generation, the browser automatically uploads the generated PDF blob to the backend. The backend stores this artifact in MinIO (S3-compatible) and links it to the immutable database record.
* **Snapshot Integrity:** The database record (JSONB snapshot) is the ultimate source of truth. The PDF is a frozen visual artifact of that truth.

### 4.3 Document Manager
* **CRUD Operations:** Create, read, update, and soft-delete documents. Updates are only allowed in `draft` status.
* **Custom Document Sequences:** Tenants can define custom document numbering patterns using a token-based system (e.g., `{{COMPANY_ID}}-{{YYYY}}-{{SEQ:4}}`). The system guarantees mathematically sequential numbering without collisions, regardless of the custom text prefix or suffix.
...
## 8. Technical Architecture (Updated High-Level)

* **Structure:** Monolith (Single Binary).
* **Backend:** **Golang (Gin Framework)** for routing, token parsing, and concurrency control.
* **Frontend:** **React (Vite)** embedded into the Go binary via `//go:embed`.
* **Database:** **PostgreSQL** (Single primary instance for v1.0 to ensure strict consistency).
* **PDF Engine:** **`@react-pdf/renderer`** — Client-side rendering utilizing the Yoga layout engine for cross-browser consistency.
* **Storage:** **MinIO** (S3-compatible) for PDF files and logos.
